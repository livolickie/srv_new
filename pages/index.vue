		...
		<v-row v-if="status != 'reload'">
			<v-col lg="2">
				<v-list dense>
					...
					<v-list-item-group>
						...
						<v-list-item @click="selectTab(4)">
							...
							<v-list-item-content> Карта сети </v-list-item-content>
						</v-list-item>
					</v-list-item-group>
				</v-list>
				...
				<v-card class="pa-2 mt-4 d-flex flex-column">
					...
					>
					<v-btn
						v-if="status != 'reload' && !startError"
						class="mb-2"
						:color="status != 'pause' ? '' : 'success'"
						@click="
							status == 'active' ? (status = 'pause') : (status = 'active')
							snack = status == 'pause'
							snackMessage = 'Система поставлена на паузу'
						"
					>
						{{ status != 'active' ? 'Возобновить' : 'Пауза' }}</v-btn
					>
			...
			<v-col>
				<v-card class="q-">
					<template v-if="tab == 0">
						...
						<v-container>
							<v-row>
								<v-col lg="2">Мощность ({{ power }} dbW):</v-col>
								<v-col>
									<v-slider
										v-model="power"
										min="50"
                ...
                <v-col>
                  <v-slider v-model="maxEnergy" min="350" max="800" step="10" thumb-label ticks></v-slider>
							...
							<h3 class="text-center">
								Затрачено: {{ spentEnergy.toFixed(1) }}W
							</h3>
						</v-container>
					</template>
					<template v-if="tab == 2">
						...
						>
							...
							<template #item.signal="{ item }">
								<span
                :class="{
                  'purple': item.obj.signal === '4G',
                  'green': item.obj.signal === '3G',
                  'grey': item.obj.signal === '2G',
                  'red': item.obj.signal === 'NO SIGNAL',
                }"
                >{{ item.obj.signal }}</span>
							</template>
						</v-data-table>
					</template>
					<template v-if="tab == 3">
                  ...
                  <v-slider v-model="maxNetwork" min="800" max="1500" step="25" thumb-label ticks></v-slider>
					...
					<template v-if="tab == 5">
						<v-scroll-x-transition>
							<h1 class="text-center">Затраты</h1>
						</v-scroll-x-transition>
						<v-container>
							<h2 class="text-center">Стоимость 1W энергии: <span class="info">1.5 руб.</span></h2>
							<h3 class="text-center">
								Затраты по энерги: {{ expenses.byEnergy.toFixed(1) }} руб.
							</h3>
							<h3 class="text-center">
								Затраты по авариям: {{ expenses.byErrors.toFixed(1) }} руб.
							</h3>
							<h3 class="text-center">
								Затраты по перезагрузкам:
								{{ expenses.byReloads.toFixed(1) }} руб.
							</h3>
						</v-container>
					</template>
				</v-card>
		...
		methods: {
			init() {
				...
				for (let i = 0; i < 2000; i++) {
					this.clients.push({
						number: `8-700-${Math.round(Math.random() * 200 + 300)}-${Math.round(
							Math.random() * 55 + 20
			...
			reload() {
				...
				this.load = 0
				this.expenses.byReloads +=
					this.clients.length * 5 + (this.clients.length * Math.random()) / 2
			...
			handleError() {
				...
				this.expenses.byErrors +=
					this.clients.length * 10 + (Math.random() * this.clients.length) / 2
				this.startError = false
			},
			errorHandler(reason = '') {
			...
			update() {
				...
				if (!this.startError) {
					if (this.power < 60) {
						...
						setTimeout(() =>
							this.errorHandler(
								'Мощность антенны слишком мала! Система поставлена паузу',
					...
					if (this.power > 160) {
						...
						setTimeout(() =>
							this.errorHandler(
								'Мощность антенны слишком высока! Система поставлена паузу',
								1500 + Math.random() * 1000
							)
						)
					}
				...
				this.expenses.byEnergy += this.power * 1.5 + Math.random() * 5
				...
				for (const client of this.clients) {
					if (Math.random() < 0.95) {
						if (client.x < 800 && Math.random() > 0.55)
							client.x += Math.random() * 5
						else client.x -= Math.random() * 2.5
					} else if (Math.random() > 9.5) {
						if (client.y < 800 && Math.random() < 0.55)
							client.y += Math.random() * 5
						else client.y -= Math.random() * 2.5
					}

					client.distance = Math.sqrt(
						Math.pow(client.x - 392, 2) + Math.pow(client.y - 392, 2)
					)
					if (client.distance <= r) {
						client.signal = '4G'
					} else if (client.distance <= r2) {
						client.signal = '3G'
					} else if (client.distance <= r3) {
						client.signal = '2G'
					} else {
						client.signal = 'NO SIGNAL'
					}
	        ...
	        Расходы денежные:
	          - Сумма за затраченный Ватт энергии
	          - Сумма за исправленные аварийные ситуации (ремонт по мощностям)
	          - Сумма за перезагрузки системы

	        Доходы:
	          - По одному тарифу за клиента - 100 рублей
	      */
				...
				this.energy = parseFloat(
					(this.power * 4.5 + 3 * Math.random()).toFixed(1)
				)
				...
				if (this.canvas) {
					...
					ctx.fill()
					ctx.fillText('Активный клиент', 64, 32 + 8)

					ctx.fillStyle = 'rgb(255, 200, 200)'
					ctx.beginPath()
					ctx.arc(32, 64, 6, 0, 2 * Math.PI)
					ctx.fill()
					ctx.fillText('Клиент вне зоны', 64, 64 + 8)

					ctx.fillStyle = 'rgb(255,255,255)'
					ctx.fillText('4G сигнал', 392, 392 + r / 1.25)
					ctx.fillText('3G сигнал', 392, 392 + r2 / 1.2)
					ctx.fillText('2G сигнал', 392, 392 + r3 / 1.15)
				}
